            ...
            <?php foreach (ActivityExtendedStatisticSections::find()->where(['activity_id' => $section->activity_id])->all() as $section_item): ?>
                <li>
                    <div class="collapsible-header <?php echo $section->id == $section_item->id ? 'active' : ''; ?>">
                        <i class="mdi-action-"></i><?php echo sprintf('%s (полей - %d)', $section_item->header, $section_item->getFieldsCount()); ?>
                    </div>
        ...
        <ul class="collapsible collapsible-accordion" data-collapsible="expandable">
            <li>
                <div class="collapsible-header active">
                    <i class="mdi-action-"></i>Кастомные функции</div>

                <div class="collapsible-body">
                    <ul id="task-card" class="collection with-header">

                        <?php foreach (ActivityExtendedStatisticFields::getCustomFunctionsList() as $key => $name): ?>
                            <li class="collection-item dismissable"
                                style="touch-action: pan-y; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
                                <input class="ch-calc-field" type="checkbox"
                                       data-id="<?php echo $key; ?>"
                                       data-name="<?php echo $name; ?>"
                                       data-section-name="Кастомные функции"
                                       id="field-id-<?php echo $key ?>" <?php echo in_array($key, $calc_fields) ? "checked" : ""; ?>>
                                <label for="field-id-<?php echo $key; ?>"
                                       style="text-decoration: none;"><?php echo $name; ?>
                                </label>
                            </li>
                        <?php endforeach; ?>
