...
<table id="model_list_by_period" class="responsive-table bordered striped">
    ...
    <tr>
        ...
        <?php $col_span_offset = []; ?>
        <?php foreach ($models_count as $hour => $count): ?>
            <?php $col_span_offset[$hour] = count($col_span_offset) + 1; ?>
            <th style="text-align: left; "><?php echo sprintf("%s ча%s", $hour, \common\utils\NumbersHelper::numberEnd($hour, array( 'с', 'са', 'сов' ))); ?></th>
        <?php endforeach; ?>
    </tr>
    </thead>
    <tbody class="">
        ...
        <?php foreach ($steps_models_list as $step_key => $step_data): ?>
            <tr style="border-left: 3px solid rgba(<?php echo $step_data[ 'color' ]; ?>);">
                <td>
                    ...
                    <p style="font-size: 0.8rem;"><?php echo sprintf("%s ча%s", $step_data['days'], \common\utils\NumbersHelper::numberEnd($step_data['days'], array( 'с', 'са', 'сов' ))); ?></p>
                </td>
    ...
    <tr>
        ...
        <?php foreach ($models_count as $hour => $count): ?>
            <td><?php echo $models_count[$hour]["count"]; ?></td>
        <?php endforeach; ?>
    </tr>
    </tfoot>
</table>
